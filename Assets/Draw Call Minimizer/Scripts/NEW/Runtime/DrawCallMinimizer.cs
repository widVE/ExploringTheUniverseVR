...
namespace DCM {
    ...
    public class DrawCallMinimizer : MonoBehaviour {
		...
		[SerializeField]
		bool makeChildOfOriginalObject = false;
		[SerializeField]
		bool destroyInactiveSiblings = false;
        [SerializeField]
        private bool
        ...
        void CreateBatchedAtlasedObjects(IDictionary<string, Dictionary<Material, List<MeshInstance>>> allMeshesAndMaterials) {
            foreach (KeyValuePair<string, Dictionary<Material, List<MeshInstance>>> firstPass in allMeshesAndMaterials) {
                ...
                if (textureCombineOutput != null && textureCombineOutput.texturePositions != null) {
					...
					GameObject parent;
					if (makeChildOfOriginalObject)
					{
						parent = gameObject;
					}
					else
					{
						parent = new GameObject("Combined " + gameObject.name + " " + firstPass.Key + " Mesh Parent");
					...
					for (int i = 0; i < combinedMeshes.Count; i++) {
						...
						if (makeChildOfOriginalObject) go.transform.SetAsFirstSibling();
                    }
        ...
        void CreateBatchedObjects(IDictionary<string, Dictionary<Material, List<MeshInstance>>> allMeshesAndMaterials) {
            ...
            {                
				...
				{
					parent = new GameObject("Combined " + gameObject.name + " " + unatlasedGroup.Key + " Mesh Parent");
					parent.transform.localScale = transform.localScale;
					parent.transform.position = transform.position;
					parent.transform.rotation = transform.rotation;
				}
		...
		void DestroyInactiveChildren()
		{
			List<GameObject> toDestroy = new List<GameObject>();
			for (int i = 0; i < transform.childCount; i++)
			{
				if (!transform.GetChild(i).gameObject.activeSelf) toDestroy.Add(transform.GetChild(i).gameObject);
			}
